...
def una(P):
    ...
    # ---- el objeto grande, entero y con margen, abajo
    # Un objeto ANCHO (el cheque es 374x181) escalado por alto se sale de cuadro y despues el
    # recorte por ancho lo deja diminuto: en la primera hoja de control el cheque medía la mitad
    # que el libro. Se escala por la dimension que manda en cada objeto.
    ob = prop(D['objeto'])
    ANCHO_MAX, ALTO_MAX = W-130, 900
    k = min(ANCHO_MAX/ob.width, ALTO_MAX/ob.height)
    ...
    if ban:
        ...
        im.alpha_composite(fl, (46, 58))

    # ---- la pregunta, arriba, en dos lineas. Es la MISMA del titulo.
    lineas = D['pregunta']
    y = 300 + (70 if D.get('bandera') else 0)
    ...
    if cif:
        ...
        texto_duro(d, (W//2, y+92), cif, f, ROJO, gr=12)
