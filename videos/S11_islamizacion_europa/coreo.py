# -*- coding: utf-8 -*-
"""Coreografia del short S03 («El aviso»). Una sola pieza; la partitura esta en `escenas.py`.
...
def imagen(name):
    """prop de la serie -> prop del ep. 06 (reuso) -> prop compartido del canal"""
    for base in (ARTE, EP06, os.path.join(PROD, 'assets')):
...
def build(n=None):
    ...
    def stamp(name, t, fx=0.5, fy=0.5, shot=None, z=85, sc_=1.0, dur=None):
        s = shot or cur[0]; x, y = at(s, fx, fy)
        im = imagen(name); S0 = 1.45
        sfit, x, y = encajar(im, sc_*S0, x, y, s, crecer=0.60)
        sc_ = sfit/S0
    ...
    API = dict(sc=sc, L=L, E=E, LN=LN, DUR=DUR, cut=cut, at=at, card=card, P=P,
               drop=drop, stamp=stamp, shake=shake, girar=girar,
