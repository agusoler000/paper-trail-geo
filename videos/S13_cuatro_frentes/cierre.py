...
# feed. Se recortaron a una frase.
#
# **CUATRO piezas, no tres** (2026-09-15). Antes la 3 cerraba la tanda y nombraba el pais
# siguiente; ahora la 3 entrega la 4 y es la 4 la que cierra. Regla 23: las cuatro frases son
# DISTINTAS — cuatro shorts seguidos en el feed con el mismo outro se oyen como un bucle.
# Las tres primeras ENTREGAN (la siguiente ya esta arriba) en vez de pedir a secas, que es el
# cambio de `canal/ENFOQUE_VISTAS_SUBS_2026-09-14.md` §3.1.
CIERRES = {
 # S13 (16-sep). Cambio sobre la S12: alli decian «PART 2 IS UP NOW». Si las piezas salen con ~2 h de
 # diferencia, eso es FALSO durante 2 h (la trampa de la skill §2.13: revisar todo texto que PROMETA).
 # «is next» es verdad en cualquier momento. Cuatro frases distintas (regla 23).
 1: ("[warm, quick] Part two is next. Like and subscribe.",
     'LIKE  +  SUBSCRIBE', 'PART 2 IS NEXT'),
 2: ("[warm, quick] Part three is next. Hit like and follow along.",
     'LIKE  +  SUBSCRIBE', 'PART 3 IS NEXT'),
 3: ("[warm, quick] One more to go. Subscribe so you don't miss it.",
     'LIKE  +  SUBSCRIBE', 'PART 4 IS NEXT'),
 4: ("[warm, quick] Four lines. No battles. Subscribe, and follow the paper.",
     'LIKE  +  SUBSCRIBE', 'THAT WAS PART 4 OF 4'),
}


def generar(n):
    """Pide el cierre a fal (eleven-v3, George) y lo guarda. **CUESTA** (~USD 0,01 por pieza).

    Es la misma voz y el mismo modelo que el cuerpo, asi que la juntura no se oye. La etiqueta de
    apertura va corta a proposito: con etiquetas largas eleven-v3 las LEE en voz alta (medido en la
    S12, ver `voz_corta.py::alinear`), y en un cierre de 3 s eso seria todo el cierre."""
    ...
    a = os.path.join(_d(n), 'audio'); os.makedirs(a, exist_ok=True)
    mp3 = os.path.join(a, 'cierre.mp3')
    open(mp3, 'wb').write(requests.get(r['audio']['url']).content)
    json.dump(r, open(os.path.join(a, 'cierre.json'), 'w'), ensure_ascii=False)
    print('cierre %d: %r -> %s (%d bytes, %.2f s)'
          % (n, txt, mp3, os.path.getsize(mp3), _dur(mp3)))
...
def pegar(n):
    ...
    if not os.path.exists(pz): raise SystemExit('sin pieza final (corre `python final.py %d`)' % n)
