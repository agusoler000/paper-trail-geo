# -*- coding: utf-8 -*-
"""Voz de la serie S13 («FOUR LINES», tanda del 2026-09-16): direccion de actor -> ElevenLabs
...
from direccion_s13 import APERTURA, TAGS
...
def alinear():
    ...
    for i, ln in enumerate(L):
        ...
        t_prev = ini
    # La ULTIMA linea no tiene vecina a la derecha y su fin se estimaba como centro + 1,6 s. Con una
    # ultima linea larga («Worth it, or not? Comments.») eso la cerraba ANTES de su ultima palabra, y
    # como el render corta en fin + 0,7 s, la pregunta final se quedaba sin audio (S13, 16-sep: en la
    # pieza 2 la linea terminaba en 79,32 s y «Comments.» sonaba en 80,35). El fin es la ultima palabra.
    if out and words:
        out[-1]['fin'] = round(min(dur - 0.05, max(out[-1]['fin'], float(words[-1][2]) + 0.15)), 3)
